...
% contact with earlier slide on non-renormalisable SUSY Lagrangian
% => may be better to use toy model in Martin's primer
\begin{frame}
  \frametitle{Example: Gravity-mediated SUSY Breaking}
  \begin{itemize}\itemsep1em
  \item Hidden sector breaking $\to$ visible sector via
    gravitational strength interactions
    \begin{itemize}
    \item Automatically present for local SUSY/SUGRA
    \item Breakdown of local SUGRA $\Rightarrow$ massive gravitino
      via super-Higgs mechanism
    \end{itemize}
  \item Toy model: visible sector $Z^I$, hidden sector $X^I$, K\"{a}hler
    potential
    \begin{equation*}
      \mathcal{G} = -z^\dagger_I z^I - H(x^I, x^\dagger_I) - M_{Pl}^2 \ln
      \frac{|W(z^I, x^I)|^2}{M_{Pl}^6}
    \end{equation*}
  \item Superpotential $W = W_o(z^I) + W_h(x^I)$ constructed with
    $\langle F_{X^I} \rangle \neq 0$
  \item $\Rightarrow$ universal soft terms
  \item Common alternatives: GMSB, AMSB, $\ldots$
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}\itemsep1em
  \item Detailed mechanism of SSB is unknown
  \item Alternative: break SUSY explicitly using (almost) most general
    set of soft interactions:
    \begin{equation*}
      -\mathcal{L}_{\text{soft}} \supset (m^2)^i{}_j \phi^*_i \phi^j +
      \left ( a_{ijk} \phi^i \phi^j \phi^k + b_{ij} \phi^i \phi^j + S_i \phi^i
      + h.c. \right ) + \frac{1}{2} \left ( M^a \lambda^a \lambda^a + h.c.
      \right )
    \end{equation*}
  \item Non-exhaustive set: Dirac gaugino masses, trilinear
